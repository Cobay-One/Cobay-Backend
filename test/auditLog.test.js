// Tests for the AuditLog entity, service and repository.

import assert from "node:assert/strict";
import { test } from "node:test";

import { AuditLog } from "../src/domain/auditLog.js";
import { AuditLogRepository } from "../src/repositories/auditLogRepository.js";
import * as service from "../src/services/auditLogService.js";

function sample() {
  return new AuditLog({ id: 1, name: "sample", amountCents: 250 });
}

test("auditLog: rename returns an independent copy", () => {
...
test("auditLog: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("auditLog: service totals and lookup", () => {
  const items = [sample(), new AuditLog({ id: 2, name: "second", amountCents: 750 })];
...
test("auditLog: repository roundtrip", () => {
