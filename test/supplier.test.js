// Tests for the Supplier entity, service and repository.

import assert from "node:assert/strict";
import { test } from "node:test";

import { Supplier } from "../src/domain/supplier.js";
import { SupplierRepository } from "../src/repositories/supplierRepository.js";
import * as service from "../src/services/supplierService.js";

function sample() {
  return new Supplier({ id: 1, name: "sample", amountCents: 250 });
}

test("supplier: rename returns an independent copy", () => {
...
test("supplier: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("supplier: service totals and lookup", () => {
  const items = [sample(), new Supplier({ id: 2, name: "second", amountCents: 750 })];
...
test("supplier: repository roundtrip", () => {
