// Tests for the Wishlist entity, service and repository.

import assert from "node:assert/strict";
import { test } from "node:test";

import { Wishlist } from "../src/domain/wishlist.js";
import { WishlistRepository } from "../src/repositories/wishlistRepository.js";
import * as service from "../src/services/wishlistService.js";

function sample() {
  return new Wishlist({ id: 1, name: "sample", amountCents: 250 });
}

test("wishlist: rename returns an independent copy", () => {
...
test("wishlist: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("wishlist: service totals and lookup", () => {
  const items = [sample(), new Wishlist({ id: 2, name: "second", amountCents: 750 })];
...
test("wishlist: repository roundtrip", () => {
