// Tests for the InventoryItem entity, service and repository.

import assert from "node:assert/strict";
import { test } from "node:test";

import { InventoryItem } from "../src/domain/inventoryItem.js";
import { InventoryItemRepository } from "../src/repositories/inventoryItemRepository.js";
import * as service from "../src/services/inventoryItemService.js";

function sample() {
  return new InventoryItem({ id: 1, name: "sample", amountCents: 250 });
}

test("inventoryItem: rename returns an independent copy", () => {
...
test("inventoryItem: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("inventoryItem: service totals and lookup", () => {
  const items = [sample(), new InventoryItem({ id: 2, name: "second", amountCents: 750 })];
...
test("inventoryItem: repository roundtrip", () => {
