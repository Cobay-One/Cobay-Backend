// Tests for the CartItem entity, service and repository.

import assert from "node:assert/strict";
import { test } from "node:test";

import { CartItem } from "../src/domain/cartItem.js";
import { CartItemRepository } from "../src/repositories/cartItemRepository.js";
import * as service from "../src/services/cartItemService.js";

function sample() {
  return new CartItem({ id: 1, name: "sample", amountCents: 250 });
}

test("cartItem: rename returns an independent copy", () => {
...
test("cartItem: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("cartItem: service totals and lookup", () => {
  const items = [sample(), new CartItem({ id: 2, name: "second", amountCents: 750 })];
...
test("cartItem: repository roundtrip", () => {
