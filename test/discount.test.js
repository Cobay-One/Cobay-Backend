// Tests for the Discount entity, service and repository.

import assert from "node:assert/strict";
import { test } from "node:test";

import { Discount } from "../src/domain/discount.js";
import { DiscountRepository } from "../src/repositories/discountRepository.js";
import * as service from "../src/services/discountService.js";

function sample() {
  return new Discount({ id: 1, name: "sample", amountCents: 250 });
}

test("discount: rename returns an independent copy", () => {
...
test("discount: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("discount: service totals and lookup", () => {
  const items = [sample(), new Discount({ id: 2, name: "second", amountCents: 750 })];
...
test("discount: repository roundtrip", () => {
