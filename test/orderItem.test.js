// Tests for the OrderItem entity, service and repository.
...
import * as service from "../src/services/orderItemService.js";

function sample() {
  return new OrderItem({ id: 1, name: "sample", amountCents: 250 });
}

test("orderItem: rename returns an independent copy", () => {
...
test("orderItem: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("orderItem: service totals and lookup", () => {
  const items = [sample(), new OrderItem({ id: 2, name: "second", amountCents: 750 })];
...
test("orderItem: repository roundtrip", () => {
