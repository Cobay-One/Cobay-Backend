// Tests for the Invoice entity, service and repository.
...
import * as service from "../src/services/invoiceService.js";

function sample() {
  return new Invoice({ id: 1, name: "sample", amountCents: 250 });
}

test("invoice: rename returns an independent copy", () => {
...
test("invoice: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("invoice: service totals and lookup", () => {
  const items = [sample(), new Invoice({ id: 2, name: "second", amountCents: 750 })];
...
test("invoice: repository roundtrip", () => {
