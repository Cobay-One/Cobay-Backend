// Tests for the Payment entity, service and repository.
...
import * as service from "../src/services/paymentService.js";

function sample() {
  return new Payment({ id: 1, name: "sample", amountCents: 250 });
}

test("payment: rename returns an independent copy", () => {
...
test("payment: deactivate and isFree", () => {
  const item = sample().deactivate().withAmount(0);
  assert.equal(item.active, false);
  assert.equal(item.isFree(), true);
});

test("payment: service totals and lookup", () => {
  const items = [sample(), new Payment({ id: 2, name: "second", amountCents: 750 })];
...
test("payment: repository roundtrip", () => {
